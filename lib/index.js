...
function installRecall(ctx, store, config) {
  ...
  const stateFor = (sessionId) => {
    ...
    if (!state) {
      state = { paths: /* @__PURE__ */ new Set(), bytes: 0, decision: null, indexText: "" };
  ...
  const memoryContext = (context) => {
    ...
    const frozen = config.freezeIndexPerSession === false ? void 0 : state.indexText || void 0;
  ...
  ctx.on("system-prompt/assemble", async (_assembly, context, next) => {
    ...
    const bundle = buildRecallBundle(
      store,
      config,
      cwd,
      query,
      ...
      pick,
      bodyMax,
      config.freezeIndexPerSession === false ? void 0 : state.indexText || void 0
    );
